...
export const FinancialsLedgerTab: React.FC<FinancialsLedgerTabProps> = ({ 
    ...
    // Changed default to 'ALL' to ensure users see their data immediately without date confusion
    ...
    useEffect(() => {
        if (filterOverride) {
            ...
            setFinancialPeriod('ALL'); // Reset period to show all pending items regardless of date
    ...
    const filteredRecords = useMemo(() => {
        return records.filter(rec => {
            ...
            // Filter logic matches simple filters in UI
    ...
    // --- Report Printing Handlers ---
    const handlePrintFinancialStatement = () => {
        const printWindow = window.open('', '_blank');
        if (!printWindow) return;

        // Calculate simplified stats for report based on filtered view
        const completed = filteredRecords.filter(r => r.status === 'COMPLETED' || !r.status);
        const totalIncome = completed.filter(r => r.type === 'INCOME').reduce((a, b) => a + b.amount, 0);
        const totalExpense = completed.filter(r => r.type === 'EXPENSE').reduce((a, b) => a + b.amount, 0);
        ...
        printWindow.document.write(`
            <html>
                ...
                <body>
                    ...
                    <p>Village: ${villageId}</p>
                    <p>Period: ${financialPeriod === 'ALL' ? 'All Time' : financialPeriod === 'MONTH' ? 'Current Month' : 'Today'}</p>
                    <p>Date Generated: ${new Date().toLocaleDateString()}</p>
                    <table>
                        <tr><th>Metric</th><th style="text-align:right">Value (RM)</th></tr>
                        <tr><td>Total Revenue</td><td style="text-align:right">${totalIncome.toLocaleString(undefined, {minimumFractionDigits: 2})}</td></tr>
                        <tr><td>Total Expenses</td><td style="text-align:right">(${totalExpense.toLocaleString(undefined, {minimumFractionDigits: 2})})</td></tr>
                        <tr class="total-row"><td>Net Profit/Loss</td><td style="text-align:right">${netProfit.toLocaleString(undefined, {minimumFractionDigits: 2})}</td></tr>
                    </table>
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            ...
            {/* Actions: Reports (Available for all Villages now) */}
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                        {/* Inject Capital Button available for all Villages (Admin/Finance) */}
                        {isFinanceOrAdmin && onInjectCapital && (
                            <button
                                onClick={onInjectCapital}
                                className="px-4 py-2 rounded-md text-sm font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 flex items-center gap-2"
                            >
                        ...
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    {isVillageC ? 'Order/Ref' : 'Batch Link'}
                                </th>
                            ...
                            {filteredRecords.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="px-6 py-10 text-center text-sm text-gray-400 italic">
                                        No transactions found. Try adjusting filters or adding a new record.
                                    </td>
                                </tr>
                            ) : (
                                            ...
                                            {(canDelete || canEdit) && (
                                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                                    <div className="flex justify-center space-x-2">
                                                        ...
                                                        {(record.status === 'COMPLETED' || !record.status) && onPrintRecord && (
                                                            <button 
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
